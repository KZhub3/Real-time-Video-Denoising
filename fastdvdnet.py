...
mylog = open('logs/'+'mylogtemp'+'.log','w')
...
#	with torch.autograd.profiler.profile(enabled=True, use_cuda=False) as prof:
	unclamped = model.forward(noisyframe, sigma_noise)

	#print(prof.table(), file=mylog)	
	# unclamped = model.forward(noisyframe, sigma_noise)
	# print("frame's  size is ",noisyframe.shape, file=mylog)
	out = torch.clamp(unclamped, 0., 1.)
...
def denoise_seq_fastdvdnet(seq, noise_std, temp_psz, model_temporal):
	...
	print(numframes,C,H,W)
	ctrlfr_idx = int((temp_psz-1)//2)
	inframes = list()

	denframes = torch.empty((numframes, C, H, W))	
	denframes = denframes.to(seq.device)
	...
	for fridx in range(numframes):
		...
		#print(fridx, "'s time is ",time5-time4, file=mylog)
		...
		#print(fridx, "frame's  time is ",time6 - time5, file=mylog)
